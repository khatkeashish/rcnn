...
from utils import Configs, getROI, non_max_suppression_fast, selectiveSearch
from voc2012 import get_labels, getFilepaths, loadAnnotation, loadImage, plot_annotations
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        dest="out_dir",
        required=False,
        help="Directory to read/write cached preprocessed files (default: ./processed)",
    )
    parser.add_argument("--regen-cache", action="store_true", help="Force regeneration of the preprocess cache")
    parser.add_argument(
        "--workers", type=int, required=False, help="Number of worker processes to use (default: 80% of CPUs)"
    )
    parser.add_argument(
        "--cache-name",
        dest="cache_name",
