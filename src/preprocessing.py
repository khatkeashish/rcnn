...
from utils import bb_intersection_over_union, getROI, selectiveSearch
from voc2012 import getFilepaths, loadAnnotation, loadImage
...
def create_dataset(
    ...
    if chunk_size and chunk_size > 0:
        ...
        while start < n:
            ...
            part_images, part_labels = _create_subset(
                idxs, image_paths, annotation_paths, image_shape, voc_labels, workers
            )
            ...
            if cache_dir and cache_name:
                ...
                try:
                    with open(part_path, "wb") as f:
                        pickle.dump(
                            {"X": np.array(part_images), "y": np.array(part_labels)},
                            f,
...
def preprocess_dataset(
    data_dir, image_shape, voc_labels, out_path=None, workers=None, cache_name=None, chunk_size=None
):
    ...
    if os.path.exists(out_path):
        try:
            ...
            print("Failed to load existing cache, regenerating...")
            pass
    ...
    if chunk_size and chunk_size > 0:
        # find part files
        parts = sorted(
            [p for p in os.listdir(cache_dir) if p.startswith(cache_name) and p.endswith(".pkl") and ".part" in p]
        )
        if parts:
            X_list = [X]
            y_list = [y]
